...
const CAM_TRIGGER_SETTING_TEXT =
  "This setting configures the source for the camera trigger.\
...
const TIMER_INTERVAL_TEXT =
  "Enter the time in seconds after which the camera should trigger repeatedly";

const OPER_MODE_HEAD = "Operating Mode";
const OPER_MODE_TEXT =
  "Choose when the camera should be triggered based on the ambient light. \
...
const TRIGGER_MODE_PIR_TEXT =
  "This setting configures how the camera is triggered. \
...
const SENSITIVITY_TEXT =
  "This setting configures how sensitive SensePi's motion sensing is. \
...
const INTERTRIGGERTIME_TEXT =
  "This setting specifies the amount of time the motion sensor should be switched off after motion detection.";
...
export class DetailPage {
  ...
  timerShowTwilightClick() {
    this.cd.detectChanges();
  }

  pirShowTwilightClick() {
    this.cd.detectChanges();
  }
  ...
  public setTimerBurstNumber() {
    console.log(
      "TIMER: Burst Number of shots is set to " + this.timerBurstNumber
    );
  }

  public setTimerBulbExposureTime() {
    console.log(
      "TIMER: Bulb Exposure Time is set to " + this.timerBulbExposureTime
    );
  }

  public setTimerVideoDuration() {
    console.log("TIMER: Video Duration is set to " + this.timerVideoDuration);
  }

  public setTimerVideoExtension() {
  ...
  public setTimerMode(event) {
    ...
    switch (+event) {
      ...
      case MODE_SETTING.TRIGGER_BULB_EXPOSURE: {
        ...
        console.log(
          "TIMER: bulbExposureTime set to " + this.timerBulbExposureTime
        );
        break;
      }
  ...
  public setTriggerTimerInterval() {
  ...
  public setPirBurstGap() {
    console.log("Pir: Burst Gap is set to " + this.pirBurstGap);
  }

  public setPirBurstNumber() {
    console.log("Pir: Burst Number of shots is set to " + this.pirBurstNumber);
  }

  public setPirBulbExposureTime() {
    console.log(
      "Pir: Bulb Exposure Time is set to " + this.pirBulbExposureTime
    );
  }

  public setPirVideoDuration() {
    console.log("Pir: Video Duration is set to " + this.pirVideoDuration);
  }

  public setPirVideoExtension() {
  ...
  public setPirSensitivity() {
    this.cd.detectChanges();
    console.log("Pir: Threshold set to " + this.pirSensitivity);
  }

  public setPirInterTriggerTime() {
  ...
  public loadDeviceConfigs(config) {
    ...
    switch (+this.pirMode) {
      ...
      case MODE_SETTING.TRIGGER_BURST: {
        this.pirBurstGap =
          dataview.getUint16(OFFSET_PIR_MODE_BURST_GAP, true) / 10;
        this.pirBurstNumber = dataview.getUint8(OFFSET_PIR_MODE_BURST_NUMBER);
        break;
      }
      case MODE_SETTING.TRIGGER_BULB_EXPOSURE: {
        this.pirBulbExposureTime =
      ...
      case MODE_SETTING.TRIGGER_VIDEO: {
        this.pirVideoDuration = dataview.getUint16(
          OFFSET_PIR_MODE_VIDEO_DURATION,
          true
        );
        this.pirVideoExtension = dataview.getUint8(
          OFFSET_PIR_MODE_VIDEO_EXTENSION
        );
        break;
      }
      default: {
    ...
    this.pirInterTriggerTime =
      dataview.getUint16(OFFSET_PIR_INTERTRIGGERTIME, true) / 10;
    ...
    switch (+this.timerMode) {
      ...
      case MODE_SETTING.TRIGGER_BURST: {
        this.timerBurstGap =
          dataview.getUint16(OFFSET_TIMER_MODE_BURST_GAP, true) / 10;
        this.timerBurstNumber = dataview.getUint8(
          OFFSET_TIMER_MODE_BURST_NUMBER
        );
        break;
      }
      case MODE_SETTING.TRIGGER_BULB_EXPOSURE: {
        this.timerBulbExposureTime =
      ...
      case MODE_SETTING.TRIGGER_VIDEO: {
        this.timerVideoDuration = dataview.getUint16(
          OFFSET_TIMER_MODE_VIDEO_DURATION,
          true
        );
        this.timerVideoExtension = dataview.getUint8(
          OFFSET_TIMER_MODE_VIDEO_EXTENSION
        );
        break;
      }
      default: {
  ...
  public loadSysInfo(info) {
    ...
    console.log(
      "sysinfoBattVolt" + this.sysinfoBattVolt + typeof this.sysinfoBattVolt
    );
  ...
  public print_settings_arraybufffer(writeBuffer: ArrayBuffer) {
    ...
    console.log(
      "triggerSetting (1 byte)= " + dataview.getUint8(OFFSET_TRIGGER_SETTING)
    );
    // == PIR Settings ====
    console.log(
    ...
    console.log(
      "PIR Threshold (1 byte)= " + dataview.getUint8(OFFSET_PIR_THRESHOLD)
    );
    console.log(
  ...
  public constructArrayBufferToWrite(): ArrayBuffer {
    ...
    if (this.pirOpertimeSetting == TIME_SETTING.DAYNIGHT_BOTH) {
      ...
      if (this.pirDNTwilight == true) {
        pirOperTimeVal =
          (DAY_TWILIGHT_THRESHOLD << 1) + (this.pirOpertimeSetting & 0x01);
      } else {
        pirOperTimeVal =
          (DAY_NIGHT_THRESHOLD << 1) + (this.pirOpertimeSetting & 0x01);
      }
    } else if (this.pirOpertimeSetting == TIME_SETTING.NIGHT_ONLY) {
      if (this.pirDNTwilight == true) {
        pirOperTimeVal =
          (NIGHT_TWILIGHT_THRESHOLD << 1) + (this.pirOpertimeSetting & 0x01);
      } else {
        pirOperTimeVal =
          (DAY_NIGHT_THRESHOLD << 1) + (this.pirOpertimeSetting & 0x01);
      }
    ...
    switch (+this.pirMode) {
      ...
      case MODE_SETTING.TRIGGER_BULB_EXPOSURE: {
        dataview.setUint16(
          OFFSET_PIR_MODE_BULB_EXPOSURE,
          (this.pirBulbExposureTime * 10) & 0xffff,
          true
        );
        dataview.setUint8(
          OFFSET_PIR_MODE_BULB_EXPOSURE + 2,
          (this.pirBulbExposureTime * 10) >> 16
        );
        break;
      }
      case MODE_SETTING.TRIGGER_VIDEO: {
    ...
    dataview.setUint16(
      OFFSET_PIR_INTERTRIGGERTIME,
      this.pirInterTriggerTime * 10,
      true
    );
    ...
    if (this.timerOpertimeSetting == TIME_SETTING.DAYNIGHT_BOTH) {
      ...
      if (this.timerDNTwilight == true) {
        timerOperTimeVal =
          (DAY_TWILIGHT_THRESHOLD << 1) + (this.timerOpertimeSetting & 0x01);
      } else {
        timerOperTimeVal =
          (DAY_NIGHT_THRESHOLD << 1) + (this.timerOpertimeSetting & 0x01);
      }
    } else if (this.timerOpertimeSetting == TIME_SETTING.NIGHT_ONLY) {
      if (this.timerDNTwilight == true) {
        timerOperTimeVal =
          (NIGHT_TWILIGHT_THRESHOLD << 1) + (this.timerOpertimeSetting & 0x01);
      } else {
        timerOperTimeVal =
          (DAY_NIGHT_THRESHOLD << 1) + (this.timerOpertimeSetting & 0x01);
      }
    ...
    switch (+this.timerMode) {
      ...
      case MODE_SETTING.TRIGGER_BURST: {
        dataview.setUint16(
          OFFSET_TIMER_MODE_BURST_GAP,
          this.timerBurstGap * 10,
          true
        );
        dataview.setUint8(
          OFFSET_TIMER_MODE_BURST_NUMBER,
      ...
      case MODE_SETTING.TRIGGER_BULB_EXPOSURE: {
        dataview.setUint16(
          OFFSET_TIMER_MODE_BULB_EXPOSURE,
          (this.timerBulbExposureTime * 10) & 0xffff,
          true
        );
        dataview.setUint8(
          OFFSET_TIMER_MODE_BULB_EXPOSURE + 2,
          (this.timerBulbExposureTime * 10) >> 16
        );
        break;
      }
      case MODE_SETTING.TRIGGER_VIDEO: {
    ...
    console.log(
      "===============SETTINGS THAT WILL BE WRITTEN TO THE DEVICE ============================"
    );
  ...
  async onButtonClickPopSettings() {
    ...
    this.timerOpertimeSetting = await this.nstorage.getItem(
      "TIMER_OPERSETTING"
    );
    ...
    this.timerBulbExposureTime = await this.nstorage.getItem(
      "TIMER_BULBEXPOSURE"
    );
    this.timerVideoDuration = await this.nstorage.getItem(
      "TIMER_VIDEODURATION"
    );
    this.timerVideoExtension = await this.nstorage.getItem(
      "TIMER_VIDEO_EXTENSION"
    );
    ...
    this.pirInterTriggerTime = await this.nstorage.getItem(
      "PIR_INTERTRIGGERTIME"
    );
  }
  ...
  onButtonClickWrite() {
    ...
    this.ble
      ...
      .catch(e => console.log("error in writing to device : " + e));
  }
